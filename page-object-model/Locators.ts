...
export class Locators {
  ...
  get table(): Locator {
    return this.onlyVisible(
      this.page.locator('//div[contains(@class,"singleTable")]'),
      true
    );
  }
  ...
  button(name: string, onlyVisible = true): Locator {
    ...
    if (!name.includes(" ") && /^[\u4e00-\u9fa5]+$/.test(name)) {
      ...
      buttonLocator = buttonLocator.filter({
        has: this.page.getByText(regex, { exact: true }),
      });
    } else {
      buttonLocator = buttonLocator.filter({
  ...
  locatorFollowingLabel(name: string, nth = -1, onlyVisible = true): Locator {
    ...
    const locator = this.page
      .locator("label")
      .filter({ has: this.page.getByText(regex, { exact: true }) })
      .nth(nth)
      .locator("xpath=/following::*[position()=1]");
  ...
  input(name: string, nth = -1, onlyVisible = true): Locator {
    return this.locatorFollowingLabel(name, nth, onlyVisible).locator(
      "input,textarea"
    );
  }
  ...
  get selectOptions(): Locator {
    return this.page.locator(
      `//div[contains(@class,'select-dropdown') and not (contains(@class,'dropdown-hidden'))]`
    );
  }
